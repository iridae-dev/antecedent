...
| Interactive graph×effect subsample | `antecedent-estimate` / `causal` | (unit) `envelope::interactive_subsample_mass_accounting_honest`; (conformance) `response_facade::interactive_graph_posterior_*` | — | Identified atoms left out of the subsample are never fit. Frequentist graph-posterior ATE and graph-posterior responses report their mass as `subsampled_out_mass` (not unidentified, not unevaluable) and stay `GraphDependent`; Bayesian envelopes still fold it into `unidentified_mass`. The `estimate.envelope.interactive_subsample` diagnostic lists the skipped atom keys and where their mass is reported |
