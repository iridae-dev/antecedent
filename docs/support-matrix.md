...
| `AverageEffect` | `Dag` | `graph_posterior` | `Bayesian` | `cheap` | internal_cross_check (`crates/antecedent/src/analysis/execute/mod.rs::prepare_graph_posterior_ate_runs_identify_per_click`) | Evidence is the same prepared-vs-fresh internal cross-check as the none cell, plus an assertion that cheap validation emits reports; it does not pin the mixture effect or validator values. Cheap runs the applicable overlap/E-value suite on the mixture. Per-atom identify on refresh. |
| `AverageEffect` | `Dag` | `graph_posterior` | `Bayesian` | `full` | internal_cross_check (`crates/antecedent/src/analysis/execute/mod.rs::prepare_graph_posterior_ate_runs_identify_per_click`) | Evidence is the same prepared-vs-fresh internal cross-check as the none cell, plus an assertion that full validation emits at least the cheap reports; it does not pin the mixture effect or validator values. This graph-posterior path runs effect refuters but does not currently attach Bayesian predictive checks or prior-sensitivity output. Per-atom identify on refresh. |
