...
| `AverageEffect` | `Pag` | `explicit` | `Bayesian` | `cheap` | internal_known_truth (`conformance/identify/generalized_adjustment`) | Cheap suite is overlap + E-value plus mixture-weighted PPC on identified completions; the fixture pins identification. Identify-per-run on refresh. |
| `AverageEffect` | `Pag` | `explicit` | `Bayesian` | `full` | internal_known_truth (`conformance/identify/generalized_adjustment`) | execute_pag_bayesian runs mixture-weighted PPC on identified completions (diagnostic refute.bayesian.ppc.envelope) and prior-sensitivity under full; effect refuters run on the mixture mean. The fixture pins identification, not the refuter/PPC numbers. Identify-per-run on refresh. |
...
| `AverageEffect` | `Pag` | `accepted` | `Bayesian` | `cheap` | internal_known_truth (`conformance/identify/generalized_adjustment`) | Cheap suite is overlap + E-value plus mixture-weighted PPC on identified completions; the fixture pins identification. Identify-per-run on refresh. |
| `AverageEffect` | `Pag` | `accepted` | `Bayesian` | `full` | internal_known_truth (`conformance/identify/generalized_adjustment`) | execute_pag_bayesian runs mixture-weighted PPC on identified completions (diagnostic refute.bayesian.ppc.envelope) and prior-sensitivity under full; effect refuters run on the mixture mean. The fixture pins identification, not the refuter/PPC numbers. Identify-per-run on refresh. |
...
| `AverageEffect` | `Dag` | `graph_posterior` | `Bayesian` | `none` | internal_cross_check (`crates/antecedent/src/analysis/execute/mod.rs::prepare_graph_posterior_ate_runs_identify_per_click`) | Evidence is an internal cross-check, not a known-truth pin: prepare_graph_posterior_ate_runs_identify_per_click asserts prepared-then-click ATE equals a fresh run() on the same hand-built graph-posterior study. Identification runs per atom inside execute; priors do not upgrade ID. Prepare compiles the plan and freezes the WeightedGraphSamples input; estimate re-identifies per atom. |
| `AverageEffect` | `Dag` | `graph_posterior` | `Bayesian` | `cheap` | internal_cross_check (`crates/antecedent/src/analysis/execute/mod.rs::prepare_graph_posterior_ate_runs_identify_per_click`) | Evidence is the same prepared-vs-fresh internal cross-check as the none cell, plus an assertion that cheap validation emits reports; it does not pin the mixture effect or validator values. Cheap runs the applicable overlap/E-value suite on the mixture plus mixture-weighted PPC. Per-atom identify on refresh. |
| `AverageEffect` | `Dag` | `graph_posterior` | `Bayesian` | `full` | internal_cross_check (`crates/antecedent/src/analysis/execute/mod.rs::prepare_graph_posterior_ate_runs_identify_per_click`) | Evidence is the same prepared-vs-fresh internal cross-check as the none cell, plus an assertion that full validation emits at least the cheap reports and attaches mixture-weighted PPC / prior-sensitivity. It does not pin the mixture effect or validator values. Per-atom identify on refresh. |
| `PulseEffect` | `TemporalDag` | `graph_posterior` | `Bayesian` | `none` | internal_cross_check (`crates/antecedent/tests/manufacturing_temporal.rs::manufacturing_dbn_posterior_bayesian_envelope`) | execute_dbn_posterior_bayesian mixes per-atom temporal effects. Evidence is an internal prepared-vs-fresh cross-check plus a loose seeded-DGP smoke envelope; it is not a frozen known-truth pin. cheap/full are refused because this path returns no refutations. Per-atom identify on refresh. |
| `SustainedEffect` | `TemporalDag` | `graph_posterior` | `Bayesian` | `none` | internal_cross_check (`crates/antecedent/tests/manufacturing_temporal.rs::manufacturing_dbn_posterior_bayesian_sustained_envelope`) | Same execute_dbn_posterior_bayesian path as Pulse DBN none; licensed form is single-step Sustained. Evidence is an internal prepared-vs-fresh cross-check plus a loose seeded-DGP smoke envelope, not a frozen known-truth pin. cheap/full are refused because this path returns no refutations. Per-atom identify on refresh. |
