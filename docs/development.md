...
Workspace and Python package version are kept in sync (currently **0.4.0**).
...
bash scripts/set_version.sh 0.4.0
...
bash scripts/tag_release.sh          # or: bash scripts/tag_release.sh 0.4.0
...
5. Tag `v0.4.0` (or bump first) to cut wheels + PyPI (+ crates.io with token).
