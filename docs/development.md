...
Workspace and Python package version are kept in sync (currently **1.8.0**).
...
Before tagging, confirm the dated 1.8.0 changelog section is present, Unreleased
is empty, its comparison link is `v1.8.0...HEAD`, and release-status text matches
...
bash scripts/set_version.sh 1.8.0
...
git commit -s -m "chore: bump version to 1.8.0"

# Tag current (or just-bumped) version and push
bash scripts/tag_release.sh          # or: bash scripts/tag_release.sh 1.8.0
...
5. Tag `v1.8.0` (or bump first) to cut wheels + PyPI (+ crates.io with token).
