...
- `crates/antecedent/tests/path_specific_edge_gformula_pins.rs` pins the
  ...
  explicit and accepted Dags, `none`/`cheap`/`full`, fresh and prepared.
...
Top-level keys: `bayesian, case, columns, contingency_table, frequentist, graph, identification, query, schema_version, truth` (10 fields).
