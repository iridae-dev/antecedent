//! Tigramite CI statistic conformance (GPDC / CMIknn / G²).
//!
...
fn fixture_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../conformance/tigramite/ci_stats")
}
...
fn tigramite_ci_stats_gpdc_cmiknn_gsquared() {
    ...
    // CMIknn — native kNN MI proxy differs in sign/scale from tigramite; check ordering.
    ...
    assert!(
        s_dep > s_ind,
        "CMIknn dep stat should exceed ind ({s_dep} vs {s_ind}); tigramite pins recorded for reference"
    );
    assert!(
        p_dep <= p_ind + 1e-9,
