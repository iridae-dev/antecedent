...
pub fn fit_glm(
    ...
    match family {
        GlmFamily::BinomialLogit => {
            fit_logistic(x_colmajor, nrows, ncols, y, backend, workspace, max_iter, tol)
        }
    }
}
...
fn fit_logistic(
    ...
    for iter in 1..=max_iter {
        ...
        for r in 0..nrows {
            ...
            // Weighted design and response: √w * X, √w * z
            z[r] *= w;
            for c in 0..ncols {
...
mod tests {
    ...
    fn logistic_separates_simple_signal() {
        // Softly separable: y mostly follows T with a little label noise.
        let n = 80usize;
        ...
        let fit = fit_glm(GlmFamily::BinomialLogit, &x, n, 2, &y, &FaerBackend, &mut ws, 100, 1e-6)
            .unwrap();
