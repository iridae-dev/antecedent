...
macro_rules! pcmci_family_builders {
    () => {
        ...
        pub fn with_fdr(mut self, fdr: bool) -> Self {
            self.fdr = fdr.then(antecedent_stats::FdrAdjustment::bh);
            self
        }

        /// Full FDR / FWER configuration.
        #[must_use]
        pub fn with_fdr_adjustment(mut self, fdr: Option<antecedent_stats::FdrAdjustment>) -> Self {
            self.fdr = fdr;
            self
        }
        ...
        pub fn with_ci(
            mut self,
            ci: std::sync::Arc<dyn antecedent_stats::ConditionalIndependence + Send + Sync>,
        ) -> Self {
