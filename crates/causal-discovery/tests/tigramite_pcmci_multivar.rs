//! Tigramite PCMCI multi-var conformance (edge set + val/p matrix entries).
//!
...
fn fixture_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../conformance/tigramite/pcmci_multivar")
}
...
#[test]
fn tigramite_pcmci_multivar_edges_and_matrices() {
    ...
    for p in tig["recovered_parents"].as_array().unwrap() {
        tig_set.insert((
    ...
    assert_eq!(
        recovered, tig_set,
        "edge-set mismatch rust={recovered:?} tigramite={tig_set:?}"
    );
    ...
    let val_matrix = tig["val_matrix"].as_array().unwrap();
    let p_matrix = tig["p_matrix"].as_array().unwrap();
    let q_matrix = tig.get("q_matrix").and_then(|v| v.as_array());
