...
fn gaussian_chain(n: usize, seed: u64) -> TabularData {
    ...
    let cols = vec![
        OwnedColumn::Float64(
            Float64Column::new(VariableId::from_raw(0), Arc::from(x0), ValidityBitmap::all_valid(n))
                .unwrap(),
        ),
        OwnedColumn::Float64(
            Float64Column::new(VariableId::from_raw(1), Arc::from(x1), ValidityBitmap::all_valid(n))
                .unwrap(),
        ),
        OwnedColumn::Float64(
            Float64Column::new(VariableId::from_raw(2), Arc::from(x2), ValidityBitmap::all_valid(n))
                .unwrap(),
        ),
    ];
...
fn parcorr_recovers_chain_skeleton() {
    ...
    let mut constraints = causal_discovery::DiscoveryConstraints::default();
    ...
    assert!(
        g.has_edge(DenseNodeId::from_raw(0), DenseNodeId::from_raw(1)),
        "expected 0—1"
    );
    assert!(
        g.has_edge(DenseNodeId::from_raw(1), DenseNodeId::from_raw(2)),
        "expected 1—2"
    );
    assert!(
