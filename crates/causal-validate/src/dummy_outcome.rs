    ...
    pub fn refute(
        ...
        if self.replicates == 0 {
            return Err(ValidationError::NotApplicable {
                message: "dummy outcome requires replicates > 0",
            });
        }
        ...
        for r in 0..self.replicates {
            fill_gaussian(&mut dummy, ctx, 0xA7E0_0008_u64.wrapping_add(u64::from(r)));
            let data = with_replaced_float(
                problem.data,
                problem.outcome(),
                Arc::<[f64]>::from(dummy.clone()),
            )?;
            let est =
        ...
        Ok(RefutationReport {
            refuter: Arc::from("dummy.outcome"),
            ...
            failure_condition: if passed {
                ...
                Some(Arc::from(format!(
                    "mean |dummy-outcome ATE|={mean_abs} exceeded threshold {}",
