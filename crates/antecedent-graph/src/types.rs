...
//!
//! [`NodeRef`] lives in `antecedent-core` so sample planning can use it without
...
pub enum Endpoint {
    ...
    Circle,
    /// Conflict (`x`). Orientation rules proposed incompatible marks.
    ///
...
impl MiddleMark {
    ...
    /// Merge an existing middle mark with an update (LPCMCI middle-mark update rule).
    #[must_use]
