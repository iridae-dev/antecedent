...
impl core::fmt::Display for AttributionError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::SizeLimit { requested, max } => {
                write!(f, "unit count {requested} exceeds max_units={max}")
            }
        }
    }
}
...
impl From<causal_model::ModelError> for AttributionError {
...
pub fn score_anomalies(
    ...
    query.validate().map_err(|e| AttributionError::Message(e.to_string()))?;
    ...
    for &target in query.targets.iter() {
        ...
        let y_all = data.float64_values(target).map_err(|e| AttributionError::Message(e.to_string()))?;
        ...
        for (pi, &p) in gather.parents.iter().enumerate() {
            ...
            let col = data.float64_values(pv).map_err(|e| AttributionError::Message(e.to_string()))?;
...
pub fn intrinsic_influence(
    ...
    // Approximate influence as difference of interventional means at parent mean±delta/2.
