//! DoWhy noisy multi-estimator conformance (`val` + `se` vs pinned references).
//!
...
fn fixture_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../conformance/dowhy/noisy_estimators")
}
...
#[test]
fn dowhy_noisy_estimators_val_and_se() {
    let expected = load_expected();
    assert_eq!(expected["tolerance_class"].as_str().unwrap(), "StableFloat");
    assert!(expected["dowhy"]["available"].as_bool().unwrap_or(false));
