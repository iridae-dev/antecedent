...
| Natural direct effect `Y₁,M₀ − Y₀,M₀` | Explicit three-node Markovian `X→M`, `X→Y`, `M→Y` DAG; tabular linear Gaussian SCM; Frequentist; no refutation suite | Shared abduced exogenous draw across both worlds; Rust and Python staged execution and artifact replay | Point-only; no sampling interval reported |
...
This matrix records the initial named nested cell. The support registry remains
the coordinate-level license; a missing implementation is not a
zero-probability counterfactual event.
