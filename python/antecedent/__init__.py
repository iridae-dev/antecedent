"""antecedent — Python bindings for the Antecedent causal engine.

Day-1 surface::

    result = antecedent.analyze(
        data,
        graph=edges,
        query=antecedent.AverageEffect(treatment="t", outcome="y"),
    )

Stage modules hold depth: ``antecedent.discovery``, ``antecedent.estimation``,
``antecedent.gcm``, ``antecedent.graph``, ``antecedent.attribution``,
``antecedent.validation``. Prefer ``Dag.from_dot`` / ``.to_dot`` (and peers);
free ``dag_from_*`` helpers remain thin aliases under ``antecedent`` and
``antecedent.graph``.

Public analysis results are nested ``AnalysisResult`` views. The native DTO
``AteAnalysisResult`` lives on ``antecedent._native`` only.

The native extension ``antecedent._native`` is an advanced FFI surface.
"""

from __future__ import annotations

from . import (
    attribution,
    counterfactual,
    data,
    design,
    discovery,
    estimation,
    extensibility,
    gcm,
    graph,
    inference,
    model,
    population,
    query,
    state,
    validation,
)
from ._analyze_handlers import analyze
from ._native import (
    Admg,
    AnomalyScores,
    ArrowLoadInfo,
    CancellationToken,
    CausalAttributionError,
    CausalCancelledError,
    CausalCompileError,
    CausalCounterfactualError,
    CausalDataError,
    CausalDesignError,
    CausalDiscoveryError,
    CausalError,
    CausalEstimateError,
    CausalGraphError,
    CausalIdentifyError,
    CausalModelError,
    CausalResourceError,
    CausalReviewError,
    CausalSerializationError,
    CausalState,
    CausalStateError,
    CausalUnsupportedError,
    CausalValidateError,
    ChangeAttributionResult,
    Contribution,
    Cpdag,
    Dag,
    DecisionEvaluation,
    DesignRanking,
    DiscoveredLink,
    FeatureRelevance,
    FittedGcm,
    GcmIteResult,
    GcmSampleResult,
    GraphEdge,
    GraphPosterior,
    MechanismChangeDetection,
    MediationEffectsSummary,
    Pag,
    PcmciDiscoveryResult,
    PosteriorArtifact,
    PredictSummary,
    RpcmciDiscoverySummary,
    TemporalCpdag,
    TemporalDag,
    TemporalPag,
    anomaly_attribution,
    attribute_distribution_change,
    attribute_distribution_change_robust,
    attribute_feature_relevance,
    attribute_path_specific,
    attribute_paths,
    attribute_structure_change,
    attribute_unit_change,
    counterfactual_ite,
    dag_from_dot,
    dag_from_gml,
    dag_from_json,
    dag_from_networkx_adjacency,
    dag_from_networkx_node_link,
    dag_to_dot,
    dag_to_gml,
    dag_to_json,
    dag_to_networkx_adjacency,
    dag_to_networkx_node_link,
    decode_model_bundle,
    decode_posterior_artifact,
    encode_model_bundle,
    encode_posterior_artifact,
    fit_gcm,
    load_float64_arrow_c_columns,
    load_float64_columns,
    mechanism_change_detection,
    mediation_effects_summary,
    predict_intervened_summary,
    rank_designs,
    rank_root_causes,
    sample_do,
    sample_interventional_distribution,
)
from .accepted_graph import AcceptedGraph
from .data import EventFrame, MultiEnvFrame, PanelFrame, event, multi_env, panel
from .design import evaluate_decision
from .discovery import (
    FCI,
    GES,
    LPCMCI,
    NOTEARS,
    PC,
    PCMCI,
    RFCI,
    RPCMCI,
    CiScreenedPosterior,
    DbnPosterior,
    ExactDagPosterior,
    JPCMCIPlus,
    LiNGAM,
    OrderMcmc,
    PCMCIPlus,
    StructureMcmc,
    discover_ci_screened_posterior,
    discover_dbn_posterior,
    discover_exact_dag_posterior,
    discover_fci,
    discover_ges,
    discover_jpcmci_plus,
    discover_lingam,
    discover_lpcmci,
    discover_notears,
    discover_order_mcmc,
    discover_pc,
    discover_pcmci,
    discover_pcmci_plus,
    discover_rfci,
    discover_rpcmci,
    discover_structure_mcmc,
    two_regime_half_split,
)
from .estimation import (
    AnalysisResult,
    ConflictSummaryView,
    EffectEnvelope,
    IdentifyResult,
    PredictiveCheckReport,
    PreparedAnalysis,
    PriorSensitivityReport,
    analyze_many,
    identify,
)
from .gcm import (
    anomaly_attribution_discovered,
    attribute_distribution_change_discovered,
    attribute_paths_discovered,
    fit_gcm_discovered,
)
from .ids import Estimator, Identifier, Latency, Refute
from .inference import Bayesian, Frequentist
from .population import (
    PopulationRegistry,
    target_all,
    target_custom_distribution,
    target_named,
    target_rows,
    target_treated,
    target_untreated,
)
from .prior_bank import (
    POPULATION_TAG_KEY,
    CompatibilityReport,
    ComposedPrior,
    ConflictPolicy,
    DesignVariable,
    EstimandFingerprint,
    ExternalPriorSourceSpec,
    ExternalPriorWeight,
    PriorCatalog,
    PriorMapping,
    PriorSource,
    PriorSourceMeta,
    TransportPolicy,
    compose_external_priors,
    populations_from_prior_sources,
)
from .query import (
    AverageEffect,
    ConditionalEffect,
    Counterfactual,
    InterventionalDistribution,
    MediationEffect,
    PathSpecificEffect,
    PulseEffect,
    SustainedEffect,
    TemporalMediationEffect,
)
from .validation import (
    validate_environment_holdout,
    validate_pcmci_alpha_sensitivity,
    validate_pcmci_block_bootstrap,
    validate_pcmci_ci_sensitivity,
    validate_pcmci_false_positive,
    validate_pcmci_lag_sensitivity,
    validate_pcmci_plus_orientation,
    validate_regime_stability,
    validate_synthetic_null_calibration,
)

__all__ = [
    # Day-1
    "AcceptedGraph",
    "Admg",
    "AnalysisResult",
    "AverageEffect",
    "Bayesian",
    "ConditionalEffect",
    "Counterfactual",
    "Cpdag",
    "Dag",
    "Estimator",
    "Frequentist",
    "Identifier",
    "Latency",
    "Refute",
    "IdentifyResult",
    "InterventionalDistribution",
    "MediationEffect",
    "Pag",
    "PathSpecificEffect",
    "PreparedAnalysis",
    "PulseEffect",
    "SustainedEffect",
    "TemporalDag",
    "TemporalMediationEffect",
    "analyze",
    "analyze_many",
    "identify",
    "fit_gcm",
    "FittedGcm",
    # Errors
    "CausalAttributionError",
    "CausalCompileError",
    "CausalCounterfactualError",
    "CausalDataError",
    "CausalDesignError",
    "CausalDiscoveryError",
    "CausalError",
    "CausalEstimateError",
    "CausalGraphError",
    "CausalIdentifyError",
    "CausalModelError",
    "CausalResourceError",
    "CausalReviewError",
    "CausalSerializationError",
    "CausalStateError",
    "CausalUnsupportedError",
    "CausalCancelledError",
    "CancellationToken",
    "CausalValidateError",
    # Stage re-exports (prefer causal.discovery / .gcm / .graph / …)
    "AnomalyScores",
    "ArrowLoadInfo",
    "ChangeAttributionResult",
    "CompatibilityReport",
    "CiScreenedPosterior",
    "Contribution",
    "DbnPosterior",
    "DecisionEvaluation",
    "DesignRanking",
    "DesignVariable",
    "DiscoveredLink",
    "ExactDagPosterior",
    "EstimandFingerprint",
    "FCI",
    "FeatureRelevance",
    "GES",
    "GcmIteResult",
    "GcmSampleResult",
    "GraphEdge",
    "GraphPosterior",
    "JPCMCIPlus",
    "LPCMCI",
    "LiNGAM",
    "MechanismChangeDetection",
    "MediationEffectsSummary",
    "NOTEARS",
    "OrderMcmc",
    "PC",
    "PCMCI",
    "PCMCIPlus",
    "PcmciDiscoveryResult",
    "PopulationRegistry",
    "PosteriorArtifact",
    "PredictSummary",
    "EffectEnvelope",
    "PredictiveCheckReport",
    "ConflictSummaryView",
    "PriorCatalog",
    "PriorMapping",
    "PriorSensitivityReport",
    "PriorSource",
    "PriorSourceMeta",
    "ComposedPrior",
    "ConflictPolicy",
    "TransportPolicy",
    "ExternalPriorSourceSpec",
    "ExternalPriorWeight",
    "POPULATION_TAG_KEY",
    "compose_external_priors",
    "populations_from_prior_sources",
    "RFCI",
    "RPCMCI",
    "RpcmciDiscoverySummary",
    "StructureMcmc",
    "TemporalCpdag",
    "TemporalPag",
    "CausalState",
    "anomaly_attribution",
    "attribute_distribution_change",
    "attribute_distribution_change_robust",
    "attribute_feature_relevance",
    "attribute_path_specific",
    "attribute_paths",
    "attribute_structure_change",
    "attribute_unit_change",
    "attribution",
    "counterfactual",
    "counterfactual_ite",
    "dag_from_dot",
    "dag_from_gml",
    "dag_from_json",
    "dag_from_networkx_adjacency",
    "dag_from_networkx_node_link",
    "dag_to_dot",
    "dag_to_gml",
    "dag_to_json",
    "dag_to_networkx_adjacency",
    "dag_to_networkx_node_link",
    "data",
    "decode_model_bundle",
    "decode_posterior_artifact",
    "design",
    "discover_ci_screened_posterior",
    "discover_dbn_posterior",
    "discover_exact_dag_posterior",
    "discover_jpcmci_plus",
    "discover_lpcmci",
    "discover_order_mcmc",
    "discover_pc",
    "discover_ges",
    "discover_lingam",
    "discover_notears",
    "discover_fci",
    "discover_rfci",
    "discover_pcmci",
    "discover_pcmci_plus",
    "discover_rpcmci",
    "discover_structure_mcmc",
    "discovery",
    "two_regime_half_split",
    "encode_model_bundle",
    "encode_posterior_artifact",
    "estimation",
    "evaluate_decision",
    "event",
    "EventFrame",
    "extensibility",
    "fit_gcm_discovered",
    "gcm",
    "graph",
    "inference",
    "load_float64_arrow_c_columns",
    "load_float64_columns",
    "mechanism_change_detection",
    "mediation_effects_summary",
    "model",
    "multi_env",
    "MultiEnvFrame",
    "panel",
    "PanelFrame",
    "population",
    "predict_intervened_summary",
    "query",
    "rank_designs",
    "rank_root_causes",
    "sample_do",
    "sample_interventional_distribution",
    "state",
    "target_all",
    "target_custom_distribution",
    "target_named",
    "target_rows",
    "target_treated",
    "target_untreated",
    "attribute_paths_discovered",
    "anomaly_attribution_discovered",
    "attribute_distribution_change_discovered",
    "validate_environment_holdout",
    "validate_pcmci_alpha_sensitivity",
    "validate_pcmci_block_bootstrap",
    "validate_pcmci_ci_sensitivity",
    "validate_pcmci_false_positive",
    "validate_pcmci_lag_sensitivity",
    "validate_pcmci_plus_orientation",
    "validate_regime_stability",
    "validate_synthetic_null_calibration",
    "validation",
    "__version__",
]

try:
    from ._native import __version__ as __version__
except ImportError:  # pragma: no cover - extension not built
    __version__ = "0.2.0"
