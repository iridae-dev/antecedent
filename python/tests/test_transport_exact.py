...
def fixture():
    ...
    identification = transport.identify_classical(
        graph, transport.SelectionDiagram("source", "target", ["y"]),
...
def test_classical_negative_is_scoped_to_selected_source():
    ...
    result = transport.identify_classical(graph, transport.SelectionDiagram("source", "target", ["y"]), outcomes=["y"], treatments=["x"])
...
def test_catalog_search_uses_available_source_when_target_formula_cannot_bind():
    ...
    catalog = transport.EvidenceCatalog(regimes=[
        transport.EvidenceRegime("experiment", "source", kind="experimental", interventions=["x"], measured=["y"]),
    ])
    law = transport.ExactDiscreteLaw(
        "source", "experiment", (("y", (0.0, 1.0)),), (0.2, 0.8), "source-1",
        interventions=(("x", 1.0),),
    )
    result = transport.evaluate_exact(identified, catalog, transport.ExactTransportData((law,)), at={"x": 1.0})
...
def test_recursive_frontdoor_exact_distribution_matches_latent_enumeration():
    ...
    probabilities = tuple(
        sum(
            mass(u, 0.4) * mass(x, 0.2 + 0.6 * u)
            * mass(m, 0.15 + 0.55 * x) * mass(y, 0.1 + 0.45 * m + 0.2 * u)
            for u in (0, 1)
        )
        for x in (0, 1) for m in (0, 1) for y in (0, 1)
    )
    catalog = transport.EvidenceCatalog(regimes=[
        transport.EvidenceRegime("observational", "target", measured=["x", "m", "y"]),
    ])
    law = transport.ExactDiscreteLaw(
        "target", "observational", tuple((v, (0.0, 1.0)) for v in ("x", "m", "y")),
        probabilities, "latent-oracle",
    )
    result = transport.evaluate_exact(identified, catalog, transport.ExactTransportData((law,)), at={"x": 1.0})
...
def test_common_prepared_lifecycle_export_consume_atomic_refresh():
    ...
    unsupported = transport.ExactTransportData((replace(data.laws[0], probabilities=(0.8, 0.2, 0.0, 0.0)),))
    with pytest.raises(ValueError):
        prepared.refresh(unsupported)
    assert prepared.inspect().execution_id == before.execution_id
    updated = transport.ExactTransportData((replace(data.laws[0], probabilities=(0.1, 0.4, 0.4, 0.1), snapshot_identity="two"),))
...
def test_catalog_standardization_uses_only_supplied_target_covariate_marginal():
    ...
    identified = transport.identify_classical(graph, transport.SelectionDiagram("source", "target", ["z"]), outcomes=["y"], treatments=["x"])
    catalog = transport.EvidenceCatalog(regimes=[
        transport.EvidenceRegime("trial", "source", kind="experimental", interventions=["x"], measured=["z", "y"]),
        transport.EvidenceRegime("covariates", "target", measured=["z"]),
    ])
    data = transport.ExactTransportData((
        transport.ExactDiscreteLaw("source", "trial", (("z", (0.0, 1.0)), ("y", (0.0, 1.0))), (0.32, 0.08, 0.12, 0.48), "trial", interventions=(("x", 1.0),)),
        transport.ExactDiscreteLaw("target", "covariates", (("z", (0.0, 1.0)),), (0.75, 0.25), "target"),
    ))
...
def test_common_prepare_entry_and_exact_contrast():
    ...
    identified, catalog, data = fixture()
    a = prepare(data, query=transport.ExactTransportQuery(identified, catalog, {"x": 1.0})).estimate()
    b = prepare(data, query=transport.ExactTransportQuery(identified, catalog, {"x": 0.0})).estimate()
...
def test_checked_irrelevant_zero_in_standardization_and_relevant_zero_failure():
    ...
    source = transport.ExactDiscreteLaw("source", "trial", (("z", (0.0, 1.0)), ("y", (0.0, 1.0))), (0.7, 0.3, 0.0, 0.0), "trial", interventions=(("x", 1.0),))
    target = transport.ExactDiscreteLaw("target", "covariates", (("z", (0.0, 1.0)),), (1.0, 0.0), "target")
    prepared = transport.prepare_exact(identified, catalog, transport.ExactTransportData((source, target)), at={"x": 1.0})
    result = prepared.estimate()
    assert result.mean("y") == pytest.approx(0.3)
    assert transport.consume_exact(prepared.export()).estimate() == result
    with pytest.raises(ValueError, match="zero_conditioning_mass"):
        prepared.refresh(transport.ExactTransportData((source, replace(target, probabilities=(0.5, 0.5)))))
...
def test_result_reasoning_and_export_retain_native_authority_after_display_edits():
    ...
    assert set(result.to_dict()["reasoning"]) >= {"identification", "support", "uncertainty", "assumptions"}
...
def test_snapshot_axis_permutation_and_shared_transform_contract():
    ...
    prepared = transport.prepare_exact(identified, catalog, data, at={"x": 1.0})
    before = prepared.inspect()
    permuted = replace(data.laws[0], axes=tuple(reversed(data.laws[0].axes)), probabilities=(0.4, 0.15, 0.1, 0.35), snapshot_identity="permuted")
...
def test_cancelled_exact_click_and_prepare_publish_no_claim():
    ...
    with pytest.raises(ValueError, match="no_execution_claim"):
        prepared.export()
    with pytest.raises(ValueError, match="cancel"):
        prepare(data, query=transport.ExactTransportQuery(identified, catalog, {"x": 1.0}), cancel=token)


def test_catalog_diagnostics_distinguish_missing_evidence_and_future_experiments():
    graph = Admg.from_edges(["x", "y"], [("x", "y")])
    identified = transport.identify_classical(graph, transport.SelectionDiagram("source", "target", []), outcomes=["y"], treatments=["x"])
    catalog = transport.EvidenceCatalog(regimes=[
...
def test_source_snapshot_walkthrough_example():
    ...
    runpy.run_path(str(Path(__file__).resolve().parents[2] / "examples/python/transport_exact.py"), run_name="__main__")
