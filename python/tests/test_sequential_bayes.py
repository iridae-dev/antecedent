...
def test_sequential_prior_dimension_mismatch():
    ...
    # Drop confounder so design ncols shrinks relative to the artifact.
    data2 = {"t": data["t"], "y": data["y"]}
    with pytest.raises(Exception, match="(?i)dimension|coefficient|ncols|prior"):
        causal.analyze(
            data2,
            graph=[("t", "y")],
            query=causal.AverageEffect(treatment="t", outcome="y"),
