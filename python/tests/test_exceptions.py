...
def test_column_length_mismatch_is_value_error():
    with pytest.raises(ValueError, match="column length"):
        antecedent.data.load_float64_columns(
            ["a", "b"],
...
def test_exceptions_subclass_causal_error():
    assert issubclass(antecedent.errors.CausalCompileError, antecedent.CausalError)
    assert issubclass(antecedent.errors.CausalDataError, antecedent.CausalError)
    assert issubclass(antecedent.errors.CausalDiscoveryError, antecedent.CausalError)
    assert issubclass(antecedent.errors.CausalAttributionError, antecedent.CausalError)
    assert issubclass(antecedent.errors.CausalDesignError, antecedent.CausalError)
    assert issubclass(antecedent.errors.CausalStateError, antecedent.CausalError)
