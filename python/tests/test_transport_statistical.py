...
def test_unknown_dependence_keeps_identification_and_withholds_interval():
    ...
    study = prepare(
        data,
        query=transport.StatisticalTransportQuery(identified, catalog, {"x": 1.0}, bootstrap=39, seed=3),
    )
...
def test_invalid_coverage_is_rejected_during_prepare(level):
    ...
    with pytest.raises(ValueError, match="coverage_level"):
        transport.prepare_statistical(identified, catalog, data, at={"x": 1.0}, coverage_level=level)
...
def test_statistical_native_authority_identity_and_load_round_trip():
    ...
    identified, catalog, data = fixture()
    study = transport.prepare_statistical(identified, catalog, data, at={"x": 1.0}, bootstrap=29, seed=41)
    result = study.estimate()
    ...
    second = transport.prepare_statistical(identified, catalog, transport.StatisticalTransportData((larger,)),
                                           at={"x": 1.0}, bootstrap=29, seed=41)
...
def test_statistical_missingness_is_not_silently_a_population_law():
    ...
    with pytest.raises(ValueError, match="missing"):
        transport.prepare_statistical(identified, catalog, transport.StatisticalTransportData((sample,)), at={"x": 1.0})
...
def test_intervention_world_resamples_do_not_collide():
    ...
    zero = replace(data.samples[0], columns={"y": [0.0] * 5 + [1.0] * 5}, interventions=(("x", 0.0),))
    two_worlds = transport.StatisticalTransportData((zero, data.samples[0]))
    study = transport.prepare_statistical(identified, catalog, two_worlds, at={"x": 0.0}, bootstrap=29)
    result = study.estimate()
    ...
    assert result.uncertainty["replicate_ids"] == list(range(29))
...
def test_joint_grid_and_contrasts_preserve_shared_replicates():
    ...
    identified, catalog, data = fixture()
    zero = replace(data.samples[0], columns={"y": [0.0] * 40 + [1.0] * 10}, interventions=(("x", 0.0),))
    points = transport.evaluate_statistical_grid(
        identified, catalog, transport.StatisticalTransportData((zero, data.samples[0])),
        at=[{"x": 0.0}, {"x": 1.0}], bootstrap=39, seed=17,
    )
...
def test_grid_failure_scope_changes_identity_and_survives_refresh():
    ...
    catalog = transport.EvidenceCatalog(
        ...
        bindings=[transport.RegimeBinding("obs", "v1", sampling="independent", dependence="independent_studies")],
        target_sampling="representative_sample",
    )
    data = transport.StatisticalTransportData((transport.RegimeSample(
        "target", "obs", "v1", {"x": [0.0] * 40 + [1.0], "y": [0.0] * 20 + [1.0] * 21},
    ),))
    single = transport.prepare_statistical(identified, catalog, data, at={"x": 0.0}, bootstrap=99, seed=17).estimate()
    points = transport.evaluate_statistical_grid(identified, catalog, data, at=[{"x": 0.0}, {"x": 1.0}], bootstrap=99, seed=17)
...
def test_convenience_target_flag_does_not_reject_source_only_estimation():
    ...
    result = transport.prepare_statistical(identified, catalog, data, at={"x": 1.0}, bootstrap=9).estimate()
    assert result.mean("y") == pytest.approx(0.8)
...
def test_sample_intervention_must_belong_to_declared_domain(value):
    ...
    with pytest.raises(ValueError, match="declared finite domain"):
        transport.prepare_statistical(
            identified, catalog, transport.StatisticalTransportData(samples=(invalid,)),
            at={"x": value}, bootstrap=0,
        )
